...
t = 3e6
t_year = [t/5 * x for x in range(5)]
angles = [0, 0.4, 1.1, 1.57, 2.2]
#angles = [0, 0.2, 0.4, 0.6, 0.8]
#angles = [0, 1, 2, 3, 4]
angles_list = []
...
prev_sail = solve_ivp(Fsail, [0, t], [AU, 0, 0, 30], rtol=1e-8, args=[0], t_eval=[t_year[0], t_year[1]])
...
for i in angles_list:
    ...
    for j in range(len(i)):
        prev_sail = solve_ivp(Fsail, [0, t], [prev_sail.y[0][1], prev_sail.y[1][1], prev_sail.y[2][1], prev_sail.y[3][1],], rtol=1e-8, args=[i[j]], t_eval=[t_year[0], t_year[1]])
...
prev_sail = solve_ivp(Fsail, [0, t], [AU, 0, 0, 30], rtol=1e-8, args=[angle], t_eval=t_year)
sail_in_x.append(prev_sail.y[0][0])
sail_in_y.append(prev_sail.y[1][0])

venus = solve_ivp(Fsun, [0, t], [0.72*AU, 0, 0, 35], rtol=1e-8, t_eval=t_year)
earth = solve_ivp(Fsun, [0, t], [AU, 0, 0, 30], rtol=1e-8, t_eval=t_year)
mars = solve_ivp(Fsun, [0, t], [1.5*AU, 0, 0, 24], rtol=1e-8, t_eval=t_year)
...
def keyclick(event):
    ...
    print(angle)
    current_sail = solve_ivp(Fsail, [0, t], [prev_sail.y[0][1], prev_sail.y[1][1], prev_sail.y[2][1], prev_sail.y[3][1],], rtol=1e-8, args=[angle], t_eval=t_year)
